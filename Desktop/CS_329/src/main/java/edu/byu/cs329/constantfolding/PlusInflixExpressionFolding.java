...
public class PlusInflixExpressionFolding implements Folding {
    class Visitor extends ASTVisitor {
        ...
        public void endVisit(InfixExpression node) {
            ...
            // evaluate
            int literal = (left + right);
            String theStringValue = String.valueOf(literal);
            // get ast, create new node, replace curr node with new node
            AST ast = node.getAST();
            ASTNode newNode = ast.newNumberLiteral(theStringValue);
    ...
    /**
     * Replaces the plus(+) literals in the tree with the literals.
     * 
     * <p>
     * Visits the root and any reachable nodes from the root to replace
     * any numeric addition expression reachable nodes containing a literal
     * with the literal itself.
     *
     * <p>
     * top := all nodes reachable from root such that each node
     * is an outermost numeric addition expression that ends
     * in a literal
     * 
     * <p>
     ...
     * 
     * <p>
     * isFoldable(n) := isNumericAdditionExpression(n)
     ...
     * @return true if numeric plus (+) literals were replaced in the rooted tree
     */
    @Override
